#include <stdio.h>
#include <stdlib.h>
#include <string.h>

typedef struct Feitico {
    char nome[50];
    int custo_mana;
    struct Feitico* proximo;
} Feitico;

typedef struct Pilha {
    Feitico* topo;
} Pilha;

Pilha* criar_pilha() {
    Pilha* p = (Pilha*)malloc(sizeof(Pilha));
    p->topo = NULL;
    return p;
}

void push(Pilha* p, const char* nome, int custo_mana) {
    Feitico* novo = (Feitico*)malloc(sizeof(Feitico));
    strcpy(novo->nome, nome);
    novo->custo_mana = custo_mana;
    novo->proximo = p->topo;
    p->topo = novo;
}

int vazia(Pilha* p) {
    return p->topo == NULL;
}

Feitico pop(Pilha* p) {
    Feitico feitico = {"", -1};
    if (vazia(p)) {
        printf("A pilha de comandos está vazia!\n");
        return feitico;
    }
    Feitico* temp = p->topo;
    feitico = *temp;
    p->topo = p->topo->proximo;
    free(temp);
    return feitico;
}

void inverter_pilha(Pilha* p) {
    Pilha* pilha_aux = criar_pilha();
    while (!vazia(p)) {
        Feitico feitico = pop(p);
        push(pilha_aux, feitico.nome, feitico.custo_mana);
    }
    while (!vazia(pilha_aux)) {
        Feitico feitico = pop(pilha_aux);
        push(p, feitico.nome, feitico.custo_mana);
    }
    free(pilha_aux);
}

void mostrar_pilha(Pilha* p) {
    if (vazia(p)) {
        printf("A pilha de comandos está vazia.\n");
        return;
    }
    Feitico* atual = p->topo;
    while (atual != NULL) {
        printf("Feitiço: %s, Custo de Mana: %d\n", atual->nome, atual->custo_mana);
        atual = atual->proximo;
    }
}

void liberar_pilha(Pilha* p) {
    while (!vazia(p)) {
        pop(p);
    }
}

int main() {
    Pilha* pilha = criar_pilha();
    int opcao;

    while (1) {
        printf("Menu:\n");
        printf("1: Adicionar um novo feitiço à pilha de comandos.\n");
        printf("2: Lançar Próximo Feitiço.\n");
        printf("3: Mostrar Pilha de Comandos.\n");
        printf("4: Inverter Ordem dos Feitiços.\n");
        printf("5: Finalizar a sequência de comandos e sair.\n");
        printf("Escolha uma opção: ");
        scanf("%d", &opcao);

        if (opcao == 1) {
            char nome[50];
            int custo_mana;
            printf("Digite o nome do feitiço e o custo de mana: ");
            scanf("%s %d", nome, &custo_mana);
            push(pilha, nome, custo_mana);
        } else if (opcao == 2) {
            Feitico feitico = pop(pilha);
            if (feitico.custo_mana != -1) {
                printf("Feitiço '%s' lançado com sucesso!\n", feitico.nome);
            }
        } else if (opcao == 3) {
            mostrar_pilha(pilha);
        } else if (opcao == 4) {
            if (vazia(pilha)) {
                printf("Não há feitiços para inverter.\n");
            } else {
                inverter_pilha(pilha);
                printf("Ordem dos feitiços invertida!\n");
            }
        } else if (opcao == 5) {
            break;
        } else {
            printf("Opção inválida. Tente novamente.\n");
        }
    }

    liberar_pilha(pilha);
    free(pilha);
    return 0;
}